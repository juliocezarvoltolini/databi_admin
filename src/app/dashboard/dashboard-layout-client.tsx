...
export default function DashboardLayoutClient({
  ...
  const [dashboradsNaoSelecionados, setDashboardsNaoSelecionados] = useState<
    Dashboard[]
  >([]);
  ...
  // Atualizar dashboards não selecionados quando pathname ou companyDashboards mudarem
  useEffect(() => {
    const dashboardsNao = companyDashboards.filter((value) => !isDashboardActive(value.id));
    setDashboardsNaoSelecionados(dashboardsNao);
  }, [pathname, companyDashboards]);
        ...
        <div className="flex flex-col h-full">
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              {!sidebarCollapsed && (
                <h1 className="text-lg font-bold text-gray-900">DataBi</h1>
              )}
            </div>
          ...
          {/* Menu de navegação */}
          <nav
            className={`flex-1 py-4 space-y-2 ${
              sidebarCollapsed ? "px-2" : "px-4"
            }`}
          >
            {/* Dashboards da Empresa */}

            {dashboradsNaoSelecionados.length > 0 && (
              <div>
                {!sidebarCollapsed && (
                ...
                <div className="space-y-1">
                  {dashboradsNaoSelecionados.map((dashboard) => (
                    <Link
                      key={dashboard.id}
          ...
          >
            <button
              ...
              className={`flex items-center w-full text-red-600 rounded-md hover:bg-red-50 transition-colors ${
                sidebarCollapsed ? "justify-center p-2" : "space-x-3 px-3 py-2"
              }`}
